...
import { LayoutList, Kanban, User, Phone, Mail, Plus, Trash2, Calendar, DollarSign, Save, ChevronDown, ChevronUp, GripVertical, Briefcase, Clock, Tag, Globe, Activity, Lock, FileText, AlignLeft } from 'lucide-react';
...
export const LeadsTab: React.FC<{ campaign: Campaign }> = ({ campaign }) => {
  ...
  const [viewMode, setViewMode] = useState<'list' | 'kanban'>(
      permissions.show_kanban ? 'kanban' : 'list'
  );
  ...
  // Edit Sheet Tabs
  const [sheetTab, setSheetTab] = useState<'details' | 'notes'>('details');
  ...
  // Validation & Handlers (kept same as before)
  const validateForm = () => {
      const hasValue = Object.keys(formData).length > 0;
      if (!hasValue) { setError('Please fill in at least one field.'); return false; }
      return true;
  };
        ...
        >
            ...
            <div className="space-y-1.5">
                {bodyKeys.map(key => {
                    ...
                    if (key === 'email') return <div key={key} className="flex items-center gap-2 text-xs text-zinc-500 dark:text-zinc-400 truncate"><Mail className="w-3 h-3 text-zinc-400 opacity-50" /> {value}</div>;
                    if (key === 'phone') return <div key={key} className="flex items-center gap-2 text-xs text-zinc-500 dark:text-zinc-400 truncate"><Phone className="w-3 h-3 text-zinc-400 opacity-50" /> {value}</div>;
                    if (key === 'revenue') return <div key={key} className="mt-2 flex items-center justify-between border-t border-dashed border-zinc-200 dark:border-white/10 pt-2"><div className="flex items-center gap-1 text-primary-600 dark:text-primary-400 text-sm font-semibold"><DollarSign className="w-3 h-3" /> {value}</div></div>;
                    return <div key={key} className="flex items-center gap-2 text-xs text-zinc-500 dark:text-zinc-400 truncate"><Activity className="w-3 h-3 text-zinc-300 opacity-50" /><span className="opacity-75">{getFieldName(key)}: {value}</span></div>;
                })}
            </div>
        </GlassCard>
      );
    ...
    <div className="space-y-6 h-full flex flex-col">
      ...
      <div className="flex justify-between items-center bg-white dark:bg-zinc-900/50 p-3 rounded-2xl border border-zinc-200 dark:border-white/5 shadow-sm">
        ...
        <div className="flex items-center gap-2">
             {/* View Toggle - Only show if both are allowed */}
             {permissions.show_kanban && permissions.show_list && (
      ...
      {!permissions.show_kanban && !permissions.show_list ? (
          <div className="flex-1 flex flex-col items-center justify-center text-zinc-500">
              ...
              <p>Lead views are currently disabled by admin.</p>
          </div>
      ) : (
          <>
            {viewMode === 'kanban' && permissions.show_kanban && (
                <>
                        ...
                        {campaign.settings.pipeline_stages.sort((a,b) => a.order - b.order).map(stage => {
                            ...
                            return (
                                <div key={stage.id} className={`rounded-xl border transition-all duration-300 overflow-hidden ${isOpen ? 'bg-zinc-50/50 dark:bg-zinc-900/30 border-primary-500/30 shadow-md' : 'bg-white dark:bg-zinc-950 border-zinc-200 dark:border-white/5'}`}>
                                    <button onClick={() => toggleAccordion(stage.id)} className="w-full flex items-center justify-between p-4">
                                        <div className="flex items-center gap-3"><div className={`w-3 h-3 rounded-full ${stage.color}`} /><span className="font-semibold text-zinc-900 dark:text-white">{stage.name}</span></div>
                                        <div className="flex items-center gap-3"><Badge>{stageLeads.length}</Badge>{isOpen ? <ChevronUp className="w-4 h-4 text-zinc-500" /> : <ChevronDown className="w-4 h-4 text-zinc-500" />}</div>
                                    </button>
                                    <div className={`grid transition-all duration-300 ease-in-out ${isOpen ? 'grid-rows-[1fr] opacity-100 pb-4' : 'grid-rows-[0fr] opacity-0'}`}><div className="overflow-hidden px-4 space-y-3">{stageLeads.length === 0 && (<p className="text-center text-sm text-zinc-500 py-2">No leads in this stage.</p>)}{stageLeads.map(lead => renderKanbanCard(lead))}</div></div>
                                </div>
                            )
                        })}
                    </div>
            ...
            {viewMode === 'list' && permissions.show_list && (
                        ...
                        <table className="w-full text-sm text-left">
                            <thead className="text-xs text-zinc-500 dark:text-zinc-400 uppercase bg-zinc-50 dark:bg-zinc-950/50 border-b border-zinc-200 dark:border-white/5">
                                <tr>
                                    ...
                                    {listColumns.map(f => <th key={f} className="px-6 py-4 font-semibold">{getFieldName(f)}</th>)}
                            ...
                            <tbody className="divide-y divide-zinc-100 dark:divide-white/5">
                                {leads.map(lead => (
                                    <tr key={lead.id} className="hover:bg-zinc-50 dark:hover:bg-white/5 transition-colors group">
                                        ...
                                        {listColumns.map(f => <td key={f} className="px-6 py-4 text-zinc-600 dark:text-zinc-400">{getFieldValue(lead, f)}</td>)}
                                        <td className="px-6 py-4"><Badge color="primary">{campaign.settings.pipeline_stages.find(s => s.id === lead.stage_id)?.name}</Badge></td>
                                        <td className="px-6 py-4 text-right"><button onClick={() => setSelectedLead(lead)} className="text-zinc-400 hover:text-primary-500">Edit</button></td>
                                    </tr>
                                ))}
                            </tbody>
                        </table>
      ...
      {/* Add Lead & Edit Sheet code remains the same... */}
      <Modal isOpen={isAddModalOpen} onClose={() => setIsAddModalOpen(false)} title="Add New Lead">
        <form onSubmit={handleAddLead} className="space-y-6">
            {error && <div className="p-3 bg-rose-500/10 border border-rose-500/20 rounded-xl text-rose-500 dark:text-rose-400 text-xs font-medium">{error}</div>}
            <div className="space-y-3"><h4 className="text-xs font-bold text-zinc-400 uppercase tracking-wider flex items-center gap-2"><User className="w-3 h-3" /> Identity</h4>{campaign.settings.active_system_fields.filter(k => ['full_name', 'title'].includes(k)).map(key => renderFormInput(key, SYSTEM_FIELDS.find(f => f.key === key)?.name || key, 'text', formData[key], (val) => setFormData({...formData, [key]: val})))}</div>
             <div className="space-y-3"><h4 className="text-xs font-bold text-zinc-400 uppercase tracking-wider flex items-center gap-2"><Phone className="w-3 h-3" /> Contact Info</h4>{campaign.settings.active_system_fields.filter(k => ['email', 'phone', 'source'].includes(k)).map(key => renderFormInput(key, SYSTEM_FIELDS.find(f => f.key === key)?.name || key, SYSTEM_FIELDS.find(f => f.key === key)?.type || 'text', formData[key], (val) => setFormData({...formData, [key]: val})))}</div>
            <div className="space-y-3"><h4 className="text-xs font-bold text-zinc-400 uppercase tracking-wider flex items-center gap-2"><DollarSign className="w-3 h-3" /> Deal Value</h4>{campaign.settings.active_system_fields.filter(k => ['revenue', 'due_date'].includes(k)).map(key => renderFormInput(key, SYSTEM_FIELDS.find(f => f.key === key)?.name || key, SYSTEM_FIELDS.find(f => f.key === key)?.type || 'text', formData[key], (val) => setFormData({...formData, [key]: val})))}</div>
            {campaign.settings.custom_fields.some(f => f.is_active) && (<div className="space-y-3 pt-2 border-t border-zinc-200 dark:border-white/10"><h4 className="text-xs font-bold text-zinc-400 uppercase tracking-wider">Additional Data</h4><div className="grid grid-cols-2 gap-3">{campaign.settings.custom_fields.filter(f => f.is_active).map(field => renderFormInput(field.key, field.name, field.type, formData[field.key], (val) => setFormData({...formData, [field.key]: val})))}</div></div>)}
      ...
      <Sheet 
        ...
        footer={
            <div className="flex justify-between w-full">
                <button onClick={handleDeleteLead} className="flex items-center gap-2 text-rose-500 hover:text-rose-600 dark:text-rose-400 dark:hover:text-rose-300 transition-colors text-sm font-medium"><Trash2 className="w-4 h-4" /> Delete Lead</button>
                <GlassButton onClick={handleUpdateLead}><Save className="w-4 h-4" /> Save Changes</GlassButton>
            </div>
        }
      >
        {selectedLead && (
            <div className="space-y-6 pb-10">
                ...
                <div className="flex items-center justify-between p-3 bg-zinc-50 dark:bg-white/5 rounded-xl border border-zinc-200 dark:border-white/5">
                    <div className="flex items-center gap-2 text-xs text-zinc-500 font-mono"><Clock className="w-3 h-3" /><span>Created: {new Date(selectedLead.created_at).toLocaleDateString()}</span></div>
                    <div className="flex items-center gap-2 text-xs text-zinc-500 font-mono"><Activity className="w-3 h-3" /><span>Updated: {new Date(selectedLead.updated_at).toLocaleDateString()}</span></div>
                </div>
                ...
                <div className="flex border-b border-zinc-200 dark:border-white/10">
                    <button 
                        ...
                        className={`px-4 py-2 text-sm font-medium border-b-2 transition-all flex items-center gap-2 ${sheetTab === 'details' ? 'border-primary-500 text-primary-500' : 'border-transparent text-zinc-500 hover:text-zinc-800 dark:hover:text-zinc-300'}`}
                    >
                        <User className="w-4 h-4" /> Lead Details
                    </button>
                    <button 
                        onClick={() => setSheetTab('notes')}
                        className={`px-4 py-2 text-sm font-medium border-b-2 transition-all flex items-center gap-2 ${sheetTab === 'notes' ? 'border-primary-500 text-primary-500' : 'border-transparent text-zinc-500 hover:text-zinc-800 dark:hover:text-zinc-300'}`}
                    >
                        <FileText className="w-4 h-4" /> Notes & History
                    </button>
                </div>

                {/* Tab Content */}
                {sheetTab === 'details' && (
                    <div className="grid grid-cols-1 md:grid-cols-2 gap-x-8 gap-y-8 animate-in fade-in slide-in-from-left-4 duration-300">
                        <div className="space-y-6">
                            <div className="space-y-3">
                                <h4 className="flex items-center gap-2 text-sm font-bold text-zinc-900 dark:text-white border-b border-zinc-200 dark:border-white/10 pb-2"><User className="w-4 h-4 text-primary-500" /> Identity</h4>
                            ...
                            <div className="space-y-3">
                                <h4 className="flex items-center gap-2 text-sm font-bold text-zinc-900 dark:text-white border-b border-zinc-200 dark:border-white/10 pb-2"><Globe className="w-4 h-4 text-blue-500" /> Contact</h4>
                        ...
                        <div className="space-y-6">
                            <div className="space-y-3">
                                <h4 className="flex items-center gap-2 text-sm font-bold text-zinc-900 dark:text-white border-b border-zinc-200 dark:border-white/10 pb-2"><DollarSign className="w-4 h-4 text-green-500" /> Deal Value</h4>
                            ...
                            {campaign.settings.custom_fields.length > 0 && (
                                <div className="space-y-3">
                                    <h4 className="flex items-center gap-2 text-sm font-bold text-zinc-900 dark:text-white border-b border-zinc-200 dark:border-white/10 pb-2"><Briefcase className="w-4 h-4 text-purple-500" /> Custom Data</h4>
                                    <div className="space-y-3">
                                        {campaign.settings.custom_fields.filter(f => f.is_active).map(field => renderFormInput(field.key, field.name, field.type, selectedLead.data[field.key], (val) => setSelectedLead({...selectedLead, data: { ...selectedLead.data, [field.key]: val }})))}
                                    </div>
                                </div>
                            )}
                        </div>
                ...
                {sheetTab === 'notes' && (
                    <div className="space-y-4 animate-in fade-in slide-in-from-right-4 duration-300 h-full">
                        <div className="flex flex-col h-full">
                            ...
                            <textarea 
                                className="w-full flex-1 min-h-[300px] bg-white dark:bg-zinc-950/50 border border-zinc-200 dark:border-white/10 rounded-xl p-4 text-sm text-zinc-900 dark:text-zinc-300 focus:outline-none focus:border-primary-500/50 resize-none shadow-sm font-sans leading-relaxed" 
